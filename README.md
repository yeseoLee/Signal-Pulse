...
관심종목을 대상으로 **5일/20일/60일 이동평균선 기반 추세**, **pivot 기반 지지/저항 가격대**, **20일/60일/120일 수익률**을 자동 계산해 Telegram과 HTML 리포트로 만드는 봇입니다.

## 개요

- 추세는 `MA5`, `MA20`, `MA60`을 사용해 `단기`, `중기`, `장기`로 나눠 제공합니다.
- 보고서의 기준 추세는 `중기(20일/60일선)` 기준으로 분류합니다.
...
- 단기 추세: `close > MA5 > MA20` 이면 `상승 추세`, `close < MA5 < MA20` 이면 `하락 추세`, 나머지는 `횡보`
- 중기 추세: `close > MA20 > MA60` 이면 `상승 추세`, `close < MA20 < MA60` 이면 `하락 추세`, 나머지는 `횡보`
- 장기 추세: `close > MA60` 이고 `120일 수익률 > 0` 이면 `상승 추세`, `close < MA60` 이고 `120일 수익률 < 0` 이면 `하락 추세`, 나머지는 `횡보`
- 보고서 섹션과 추세 변화 추적은 `중기 추세`를 기준값으로 사용합니다.
...
- 종목별로 `단기/중기/장기 추세`, 20/60/120일 수익률, 지지, 저항을 보여줍니다.
...
- 구현 함수와 시그널 구조: [`docs/developer-signals.md`](./docs/developer-signals.md)
